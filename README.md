# throughline-tone-formal

The **formal register** of the **tone / register** content axis, expressed as a
...
(conventions). This source is **only** the tone axis, and **only** its **formal**
register: measured, impersonal and authoritative. It says nothing about:
...
(`throughline-tone-neutral`, `throughline-tone-informal`) and a consumer composes
exactly one under the `tone` namespace — swapping register is a one-line `url`/`ref`
change. A task like *"a plain, formal, UK-English web page"* becomes a **compose** of
`plain` + `conventions-uk` + `tone-formal`.
